...
const App = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5005/records').then(response => {
      setRecords(response.data);
    });
  }, []);

  const addRecord = (newRecord) => {
    axios.post('http://localhost:5005/records', newRecord)
    .then(response => {
  ...
  const updateStatus = (id, status) => {
    ...
    axios.put(`http://localhost:5005/records/${id}`, updatedRecord).then(response => {
